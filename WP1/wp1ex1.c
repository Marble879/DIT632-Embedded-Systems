...
int main(void)
{
    ...
    do
    {
        ...
        if (scanf("%d%c", &menuOption, &term) == 0 || term != '\n') // Takes the input of the user. First check ensures that TODO: <input explanation>. Second check ensures that \n is not considered an invalid input as it is by default in buffer. Reference: https://stackoverflow.com/questions/4072190/check-if-input-is-integer-type-in-c
        {
